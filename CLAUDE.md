...
1. **Maximize simplicity, minimize complexity.** Simpler is better. Weigh complexity cost against improvement magnitude.
2. **All signal, zero noise.** Everything must earn its place. If it doesn't add value, remove it.
3. **Document every discovery.** Write insights immediately in the appropriate place (CLAUDE.md, chronicles, plans).
...
4. **Make CLAUDE.md a cheat sheet, not a novel.** Tables, code snippets, direct statements. Maximize density. See Pillar #2.
